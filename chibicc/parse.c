...
static Node *funcall(Token **rest, Token *tok, Node *fn) {
  ...
  *rest = skip(tok, ")");
  ...
  // Tarka-C Logic: Safe Gateways
  if (fn->kind == ND_VAR && fn->var && fn->var->name) {
      bool is_gateway = false;
      if (strcmp(fn->var->name, "memcpy") == 0) { fn->var->name = "__tarka_safe_memcpy"; is_gateway = true; }
      else if (strcmp(fn->var->name, "memset") == 0) { fn->var->name = "__tarka_safe_memset"; is_gateway = true; }
      else if (strcmp(fn->var->name, "strcpy") == 0) { fn->var->name = "__tarka_safe_strcpy"; is_gateway = true; }
      else if (strcmp(fn->var->name, "malloc") == 0) { fn->var->name = "tarka_malloc"; is_gateway = true; }
      else if (strcmp(fn->var->name, "free") == 0) { fn->var->name = "tarka_free"; } // free doesn't need extra args for the gateway yet

      if (is_gateway) {
          // Add extra args: filename and line number
          char *filename = (tok->file) ? tok->file->name : "unknown";
          Node *file = new_var_node(new_string_literal(filename, array_of(ty_char, strlen(filename) + 1)), tok);
          Node *line = new_num(tok->line_no, tok);
  ...
  // Tarka-C Logic: Auto-Nullification for free/tarka_free
  if (fn->kind == ND_VAR && fn->var && fn->var->name && 
      (strcmp(fn->var->name, "tarka_free") == 0 || strcmp(fn->var->name, "free") == 0)) {
      if (node->args && !node->args->next && node->args->kind == ND_VAR) {
          Node *assign = new_binary(ND_ASSIGN, node->args, new_num(0, tok), tok);
          return new_binary(ND_COMMA, node, assign, tok);
      }
  }
