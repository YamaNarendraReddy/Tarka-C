...
static bool tarka_enabled = true;
...
static void gen_check(char *reg, Node *node) {
  if (!tarka_enabled) return;
  ...
  println("  push %%r10");
  println("  push %%r11");
  println("  mov %s, %%rdi", reg);
  
  // Pass filename and line number for diagnostics
  char *filename = (node && node->tok && node->tok->file) ? node->tok->file->name : "unknown";
  int line_no = (node && node->tok) ? node->tok->line_no : 0;

  int label = count();
  println("  .section .rodata");
  println(".L.tarka.file.%d:", label);
  println("  .string \"%s\"", filename);
  println("  .text");
  println("  lea .L.tarka.file.%d(%%rip), %%rsi", label);
  println("  mov $%d, %%rdx", line_no);
  
  println("  mov $0, %%rax");
  println("  call __tarka_check@PLT");
  println("  pop %%r11");
  println("  pop %%r10");
  println("  pop %%r9");
  println("  pop %%r8");
...
static void load(Type *ty, Node *node) {
  switch (ty->kind) {
  case TY_ARRAY:
  case TY_STRUCT:
  case TY_UNION:
...
static void store(Type *ty, Node *node) {
  pop("%rdi");
  gen_check("%rdi", node);
...
static void gen_expr(Node *node) {
  ...
  case ND_CAS: {
    ...
    println("  mov %%rax, %%r8");
    load(node->cas_old->ty->base, node);
    pop("%rdx"); // new
    pop("%rdi"); // addr
  ...
  case ND_ADD:
    ...
    if (!node->ty->is_unsigned && (node->ty->kind == TY_INT || node->ty->kind == TY_LONG)) {
      println("  jo .L.tarka.overflow");
    }
...
static void emit_text(Obj *prog) {
  for (Obj *fn = prog; fn; fn = fn->next) {
    ...
    // Tarka-C: Global Initialization (Auto-call in main)
    if (strcmp(fn->name, "main") == 0) {
      println("  call __tarka_init_globals@PLT");
    }

    // Tarka-C: Register local variables and parameters
    for (Obj *var = fn->params; var; var = var->next) {
      if (var->ty->size <= 0) continue;
      int label = count();
      println("  .section .rodata");
      println(".L.tarka.param.%d:", label);
      println("  .string \"%s\"", var->name);
      println("  .text");
      println("  lea %d(%%rbp), %%rdi", var->offset);
      println("  mov $%d, %%rsi", var->ty->size);
      println("  mov $0, %%rdx");
      println("  lea .L.tarka.param.%d(%%rip), %%rcx", label);
    ...
    for (Obj *var = fn->locals; var; var = var->next) {
      ...
      println("  lea .L.tarka.var.%d(%%rip), %%rcx", label);
      println("  call __tarka_register@PLT");
    }
    ...
    // Tarka-C: Unregister local variables
    // We save rax because it contains the return value
    println("  push %%rax"); 
    for (Obj *var = fn->locals; var; var = var->next) {
      if (var->ty->size <= 0) continue;
      println("  lea %d(%%rbp), %%rdi", var->offset);
      println("  call __tarka_unregister@PLT");
    }
    println("  pop %%rax");
...
void codegen(Obj *prog, FILE *out) {
  ...
  for (Obj *var = prog; var; var = var->next) {
    ...
    // We register everything that has an address
